...
//        System.out.println("UnRegister Master Stream Size: "+streams.size());
    ...
    private void mixImages(Collection<Frame> frames, Frame targetFrame) {
        ...
        BufferedImage targetImage = targetFrame.getImage();
        if (targetImage != null) {
            ...
            do {
                ...
                boolean isAccelerated = image.getCapabilities(gc).isAccelerated();
                ...
                // Finally, copy the VolatileImage to targetImage
                Graphics2D targetG = targetImage.createGraphics();

                targetG.setComposite(getInstance(SRC, 1));
                targetG.drawImage(image, 0, 0, null);
                targetG.dispose();
